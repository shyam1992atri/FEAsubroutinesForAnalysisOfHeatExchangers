...
mf=[-174.765;457.73;-748.874;610.8;-266.265];           % this matrix initialises the m.cp values of all the streams

boundry_constant=[170;300;180;300;160];     % This matrix initialisrs the boundry conditions specified in the problem
h=[94 116 163 135 104];
fin_hight=[0.01 0.019 0.019 0.015 0.01];
fin_freq=[244.1 78.74 118.5 250.1 244.1];
alfa=[0.1 0.2 0.3 0.2 0.1];
aw=0.5*1;   %length=1meter bredth=0.5 meter
af=zeros(1,n);

mdot=[0.191 0.455 0.718 0.6 0.291];  %this array gives mass flow rate values
fin_thick=[0.254e-3 0.813e-3 0.813e-3 0.454e-3 0.254e-3];
